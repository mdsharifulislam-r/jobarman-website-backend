...
const updateApplicationStatusToDB = async (id: string, status: APPLICATION_STATUS,body:IApplication) => {
    ...
    if(status === APPLICATION_STATUS.SHORTLISTED){
        await Application.findOneAndUpdate({ _id: id }, { status: APPLICATION_STATUS.SHORTLISTED,$push:{history:{title:'Shortlisted',date:new Date(),description:'Application has been shortlisted'}} }, { new: true });
    }
    ...
    if(status === APPLICATION_STATUS.INTERVIEW){
        const data = await Application.findOneAndUpdate({ _id: id }, { status: status,$push:{history:{title:'Interview',date:new Date(),description:'Application has been sent for interview',}},interviewDetails:body.interviewDetails,isInterviewCompleted:false, }, { new: true });
        ...
        await kafkaProducer.sendMessage("notification",{
            ...
            message:`Your application for ${data?.title} has been sent for interview on ${new Date(data!?.interviewDetails!?.date).toLocaleDateString()} at ${body?.interviewDetails?.time}!`,
            isRead:false,
            receiver:[data?.user],
...
const deleteApplicationFromDB = async (id:Types.ObjectId,user:JwtPayload)=>{
    console.log(id,user);
    const data = user.role =='RECRUITER' ? {reqruiter_deleted:true} : {user_deleted:true}
    console.log(data);
...
const feedBackOfInterview = async (id:Types.ObjectId,body:Partial<IApplication>)=>{
  const result =  await Application.findByIdAndUpdate(id,{...body,isInterviewCompleted:true,$push:{history:{title:'Status',date:new Date,description:body.inteviewStatus}}},{new:true})
    await RedisHelper.keyDelete(`applications:${result?.recruiter}:*`)
    return true
}


const autoApplyForJobPosts = async (user:JwtPayload,cvPath:string,title:string,persentage:number,autoApplyId:string) => {
   try {
    console.log(cvPath);
    ...
    const similerpost = (await Post.find({
        // title:{ $regex: new RegExp(title, 'i') },
        // required_skills: { $in: skills },
        deadline: { $gte: new Date() },
        status: 'active',
        // _id: { $nin: applications.map(app => app.post) }
    }).lean().exec()).map(post=>({...post,_id:post._id.toString(),recruiter:post.recruiter.toString(),category:post.category.toString()}));
    
    const postIds = await AIHelper.getJobMatchAutoApplyPersentances(user.id,persentage,aiFile!,similerpost as any);
    ...
    for(const postId of postIds){
     try {
      ...
      io.emit(`auto-apply-progress::${autoApplyId}`, {
        ...
        posts:postIds?.filter((v:any)=>v._id!== (postId as any)._id).slice(0,5)
      })
     } catch (error) {
    ...
    await AutoApply.findByIdAndUpdate(autoApplyId,{totalApplied:postIds.length,successfulApplied:completedCount,posts:postIds.map((post:any)=>post._id)},{new:true});
    return true;
   } catch (error) {
    console.log(error);
