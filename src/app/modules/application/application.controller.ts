...
const createApplication = catchAsync(async (req: Request, res: Response) => {
  ...
  if (
    ...
    throw new ApiError(
      403,
      'You have reached the maximum number of applications for this month. Please upgrade your subscription to use this feature.',
    );
  }
  ...
  await kafkaProducer.sendMessage('application', {
    type: 'create',
...
const updateStatusOfApplications = catchAsync(
  async (req: Request, res: Response) => {
    ...
    if (body.status === APPLICATION_STATUS.INTERVIEW) {
      ...
      body.interviewDetails.date = new Date(
        `${body.interviewDetails.date} ${body.interviewDetails.time}`,
      );
    }
...
const sendFeedBackofInterview = catchAsync(
  async (req: Request, res: Response) => {
    const { id } = req.params;
    const application = await Application.findById(id);
    const body = req.body;
...
const autoApplyFeaturesForUser = catchAsync(
  async (req: Request, res: Response) => {
    ...
    const thisMonthAutoApplies = await AutoApply.countDocuments({
      user: (req.user as any).id,
      createdAt: {
        $gte: new Date(new Date().getFullYear(), new Date().getMonth(), 1),
        $lt: new Date(new Date().getFullYear(), new Date().getMonth() + 1, 1),
    ...
    if (thisMonthAutoApplies >= 5 && (!subscription?.isPremium)) {
      throw new ApiError(403, 'You have reached the maximum number of auto applies for this month. Please upgrade your subscription to use this feature.');
    }
    ...
    const isPremiumUser = await subscriptionHelper.isPremiumUser(
      (req.user as any).id,
    );
    if (!isPremiumUser) {
      throw new ApiError(
...
const autoApplyResultsForUser = catchAsync(
  async (req: Request, res: Response) => {
    const { id } = req.params;
...
const recentApplications = catchAsync(async (req: Request, res: Response) => {
  const result = await ApplicationServices.getRecentApplications(
    req.query,
    req.user as any,
  );
  sendResponse(res, {
...
const getApplicationsByUser = catchAsync(
  async (req: Request, res: Response) => {
    const result = await ApplicationServices.getUserApplications(
      req.user as any,
      req.query,
    );
    sendResponse(res, {
...
const startInterview = catchAsync(async (req: Request, res: Response) => {
  const { id } = req.params;
  const result =
    await ApplicationServices.startExtarnerNalInterviewOfApplication(id);
  sendResponse(res, {
    statusCode: StatusCodes.OK,
    success: true,
...
const changeTimedateOfIterview = catchAsync(
  async (req: Request, res: Response) => {
    const { id } = req.params;
    let data = req.body;
    ...
    await kafkaProducer.sendMessage('application', {
      type: 'changeInterviewDetails',
      data: { _id: id, data: data },
    });
    sendResponse(res, {
      statusCode: StatusCodes.OK,
      success: true,
...
const cancelInterview = catchAsync(async (req: Request, res: Response) => {
  ...
  await kafkaProducer.sendMessage('application', {
    ...
    data: { _id: id, data: req.body.reason },
  });
  sendResponse(res, {
    statusCode: StatusCodes.OK,
    success: true,
