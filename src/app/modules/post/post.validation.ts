...
const createPostZodSchema = z.object({
  ...
  min_salary: z.string().refine((v) => Number(v) >= 0, "min_salary must be >= 0"),
  max_salary: z.string().refine((v) => Number(v) >= 0, "max_salary must be >= 0"),
  location: z.string().min(2),
  required_skills: z.array(z.string().min(1)).optional().default([]),
  deadline: z.preprocess((arg) => {
    if (typeof arg === "string" || arg instanceof Date) {
        if(new Date(arg) < new Date()) {
            throw new ApiError(StatusCodes.BAD_REQUEST, 'Deadline must be greater than current date');
        }
        return new Date(arg);
    }
...
const updatePostZodSchema = z.object({
  ...
  category: z.string().regex(/^[a-f\d]{24}$/i, "Invalid ObjectId").optional(),
  ...
  min_salary: z.string().refine((v) => Number(v) >= 0, "min_salary must be >= 0" ).optional(),
  max_salary: z.string().refine((v) => Number(v) >= 0, "max_salary must be >= 0").optional(),
